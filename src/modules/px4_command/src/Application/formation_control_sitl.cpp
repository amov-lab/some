...
mavros_msgs::State current_state_uav5;
Eigen::Vector3d pos_drone_uav5;
Eigen::Vector3d vel_drone_uav5;
...
Eigen::Vector3d Takeoff_position_uav5 = Eigen::Vector3d(0.0,0.0,0.0);
...
Eigen::Vector3d delta_uav1 = Eigen::Vector3d(0,0.0,0.0);
Eigen::Vector3d delta_uav2 = Eigen::Vector3d(0,0,0.0);
Eigen::Vector3d delta_uav3 = Eigen::Vector3d(0,0,0.0);
Eigen::Vector3d delta_uav4 = Eigen::Vector3d(0,0,0.0);
Eigen::Vector3d delta_uav5 = Eigen::Vector3d(0,0,0.0);

Eigen::Vector3d body_uav1 = Eigen::Vector3d(0,0,0.0);
Eigen::Vector3d body_uav2 = Eigen::Vector3d(0,0,0.0);
Eigen::Vector3d body_uav3 = Eigen::Vector3d(0,0,0.0);
Eigen::Vector3d body_uav4 = Eigen::Vector3d(0,0,0.0);
Eigen::Vector3d body_uav5 = Eigen::Vector3d(0,0,0.0);

Eigen::Vector3d formation_uav1 = Eigen::Vector3d(0,0,0.0);
Eigen::Vector3d formation_uav3 = Eigen::Vector3d(0,0,0.0);
Eigen::Vector3d formation_uav4 = Eigen::Vector3d(0,0,0.0);
...
mavros_msgs::PositionTarget pos_setpoint5;
...
{
    Command_Now = *msg;
		if(Command_Now.Mode == command_to_mavros::Move_Body)
		{
									
			body_uav1[0] = pos_drone_uav1[0];
			body_uav1[1] = pos_drone_uav1[1];
			body_uav1[2] = pos_drone_uav1[2];

			body_uav2[0] = pos_drone_uav2[0];
			body_uav2[1] = pos_drone_uav2[1];
			body_uav2[2] = pos_drone_uav2[2];

			body_uav3[0] = pos_drone_uav3[0];
			body_uav3[1] = pos_drone_uav3[1];
			body_uav3[2] = pos_drone_uav3[2];

			body_uav4[0] = pos_drone_uav4[0];
			body_uav4[1] = pos_drone_uav4[1];
			body_uav4[2] = pos_drone_uav4[2];

			body_uav5[0] = pos_drone_uav5[0];
			body_uav5[1] = pos_drone_uav5[1];
			body_uav5[2] = pos_drone_uav5[2];

		}
}

void formation_change_cb(const std_msgs::Int8::ConstPtr& msg)
{
		switch(msg->data)
		{
			case course:
                formation_uav1 = Eigen::Vector3d(0,0,0);
                formation_uav3 = Eigen::Vector3d(0,0,0);
                formation_uav4 = Eigen::Vector3d(0,0,0);
                formation_change_flag = course;
                break;

            case triangle:
                formation_uav1 = Eigen::Vector3d(2,0,0);
                formation_uav3 = Eigen::Vector3d(1,0,0);
                formation_uav4 = Eigen::Vector3d(1,0,0);
                formation_change_flag = triangle;
                break;
		}
}
...
void uav5_state_sub_cb(const mavros_msgs::State::ConstPtr &msg)
{
    current_state_uav5 = *msg;
}

void uav5_position_sub_cb(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    pos_drone_uav5  = Eigen::Vector3d(msg->pose.position.x,msg->pose.position.y,msg->pose.position.z);
}

void uav5_velocity_sub_cb(const geometry_msgs::TwistStamped::ConstPtr &msg)
{
    vel_drone_uav5 = Eigen::Vector3d(msg->twist.linear.x,msg->twist.linear.y,msg->twist.linear.z);
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber Command_sub = nh.subscribe<px4_command::ControlCommand>("/px4_command/control_command", 10, Command_cb);
		ros::Subscriber formation_change_sub = nh.subscribe<std_msgs::Int8>("/formation/change", 10, formation_change_cb);
    ...
    ros::Subscriber uav5_state_sub = nh.subscribe<mavros_msgs::State>("/uav5/mavros/state", 10, uav5_state_sub_cb);
    ros::Subscriber uav5_position_sub = nh.subscribe<geometry_msgs::PoseStamped>("/uav5/mavros/local_position/pose", 100, uav5_position_sub_cb);
    ros::Subscriber uav5_velocity_sub = nh.subscribe<geometry_msgs::TwistStamped>("/uav5/mavros/local_position/velocity_local", 100, uav5_velocity_sub_cb);
    ros::Publisher uav5_setpoint_raw_local_pub = nh.advertise<mavros_msgs::PositionTarget>("/uav5/mavros/setpoint_raw/local", 10);
    ...
    mavros_msgs::SetMode uav5_mode_cmd;
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>主  循  环<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    while(ros::ok())
    {
        ...
        case command_to_mavros::Idle:
            ...
            pos_setpoint5.header.stamp = ros::Time::now();
            pos_setpoint1.type_mask = 0x4000;
            pos_setpoint2.type_mask = 0x4000;
            pos_setpoint3.type_mask = 0x4000;
            pos_setpoint4.type_mask = 0x4000;
            pos_setpoint5.type_mask = 0x4000;
            ...
            uav5_setpoint_raw_local_pub.publish(pos_setpoint5);
            ...
            {
                uav5_arm_cmd.request.value = true;
                uav5_arming_client.call(uav5_arm_cmd);
            ...
            pos_setpoint5.type_mask = 0b100111111000;  // 100 111 111 000  xyz + yaw
            ...
            pos_setpoint5.coordinate_frame = 1;
            ...
            pos_setpoint4.position.x = pos_sp[0] + delta_uav4[0];
            pos_setpoint4.position.y = pos_sp[1] + delta_uav4[1];
            pos_setpoint4.position.z = pos_sp[2] + delta_uav4[2];

            pos_setpoint5.position.x = pos_sp[0] + delta_uav5[0];
            pos_setpoint5.position.y = pos_sp[1] + delta_uav5[1];
            pos_setpoint5.position.z = pos_sp[2] + delta_uav5[2];

            pos_setpoint1.yaw = yaw_sp;
            pos_setpoint2.yaw = yaw_sp;
            pos_setpoint3.yaw = yaw_sp;
            pos_setpoint4.yaw = yaw_sp;
            pos_setpoint5.yaw = yaw_sp;
        ...
        case command_to_mavros::Move_Body:
            ...
            pos_setpoint1.position.x = pos_sp[0] + body_uav2[0] + formation_uav1[0];
            pos_setpoint1.position.y = pos_sp[1] + body_uav1[1];
            pos_setpoint1.position.z = pos_sp[2] + body_uav1[2];


            pos_setpoint2.position.x = pos_sp[0] + body_uav2[0];
            pos_setpoint2.position.y = pos_sp[1] + body_uav2[1];
            pos_setpoint2.position.z = pos_sp[2] + body_uav2[2];

            pos_setpoint3.position.x = pos_sp[0] + body_uav2[0] + formation_uav3[0];
            pos_setpoint3.position.y = pos_sp[1] + body_uav3[1];
            pos_setpoint3.position.z = pos_sp[2] + body_uav3[2];

            pos_setpoint4.position.x = pos_sp[0] + body_uav5[0] + formation_uav4[0];
            pos_setpoint4.position.y = pos_sp[1] + body_uav4[1];
            pos_setpoint4.position.z = pos_sp[2] + body_uav4[2];

            pos_setpoint5.position.x = pos_sp[0] + body_uav5[0];
            pos_setpoint5.position.y = pos_sp[1] + body_uav5[1];
            pos_setpoint5.position.z = pos_sp[2] + body_uav5[2];
...
void prinft_drone_state(float current_time)
{
    ...
    cout <<">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>UAV5 State<<<<<<<<<<<<<<<<<<<<<<<<<<<" <<endl;

    cout << "Time: " << current_time <<" [s] ";

    //是否和飞控建立起连接
    if (current_state_uav5.connected == true)
    {
        cout << " [ Connected ]  ";
    }
    else
    ...
    //是否上锁
    if (current_state_uav5.armed == true)
    {
        cout << "  [ Armed ]   ";
    }
    else
    ...
    cout << " [ " << current_state_uav5.mode<<" ]   " <<endl;

    cout << "Position_uav5 [X Y Z] : " << pos_drone_uav5[0] << " [ m ] "<< pos_drone_uav5[1]<<" [ m ] "<<pos_drone_uav5[2]<<" [ m ] "<<endl;
    cout << "Velocity_uav5 [X Y Z] : " << vel_drone_uav5[0] << " [m/s] "<< vel_drone_uav5[1]<<" [m/s] "<<vel_drone_uav5[2]<<" [m/s] "<<endl;
}
