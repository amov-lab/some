...
int main(int argc, char** argv){
  ...
  ros::Publisher camera_pose_publisher = node.advertise<geometry_msgs::PoseStamped>("vision_xyz", 10);
  ...
  //double output_rate = 30, roll_cam = 0, pitch_cam = 0, yaw_cam = 1.5707963, gamma_world = 1.5707963;//by sjj launch 有定义
  ...
  while (node.ok())
  {
    try
    {
      ...
      if (last_tf_time < transform.stamp_)
      {
        ...
        //printf data for test by sjj 8.24
        ROS_INFO("origin x:%f;origin y:%f,origin z:%f",position_orig.getX(),position_orig.getY(),position_orig.getZ());
        ROS_INFO("   tf_ x:%f,    tf y:%f,    tf z:%f",msg_body_pose.pose.position.x,msg_body_pose.pose.position.y,msg_body_pose.pose.position.z);
        //ROS_INFO("   tf_ -x:%f,    tf y:%f,    tf z:%f",-msg_body_pose.pose.position.x,msg_body_pose.pose.position.y,msg_body_pose.pose.position.z);
